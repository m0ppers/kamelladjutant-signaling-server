    ...
    return {
        ...
        createConnectedStateCallbacks: function(client, clients) {
            return {
                ...
                "initiate": function(data, initiateReplyFn) {
                    var peers = clients.filter(roommates(client)).filter(function(roommate) {
                        return roommate.id == data.id;
                    });
                    if (peers.length == 1) {
                        var peerData = {"id": client.id, "offerSDP": data.offerSDP, "offerCandidates": data.offerCandidates};
                        peers[0].mopRpc.send("initiate", peerData, function(answerRTCData) {
                            initiateReplyFn(answerRTCData, {"replyTimeout": 20000});
                        }, {"replyTimeout": 20000});
                    } else {
